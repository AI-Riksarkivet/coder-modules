...
if [ -n "${SERVER_BASE_PATH}" ]; then
    uv run --python "$${MARIMO_VENV}/bin/python" marimo edit --headless --watch --host 0.0.0.0 --port ${PORT} --no-token --sandbox my_notebook.py --base-url=${SERVER_BASE_PATH} >> ${LOG_PATH} 2>&1 &
else
